...
use \Exception;
use \PDO;
use News\News as ChildNews;
use News\NewsQuery as ChildNewsQuery;
use News\Map\NewsTableMap;
 ...
 * @method     ChildNewsQuery orderByHeadline($order = Criteria::ASC) Order by the headline column
 *
 * @method     ChildNewsQuery groupById() Group by the id column
 * @method     ChildNewsQuery groupByHeadline() Group by the headline column
 *
 ...
 * @method     ChildNews|null findOneByHeadline(string $headline) Return the first ChildNews filtered by the headline column
 *
 ...
 * @method     ChildNews requireOneByHeadline(string $headline) Return the first ChildNews filtered by the headline column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildNews[]|Collection findByHeadline(string|array<string> $headline) Return ChildNews objects filtered by the headline column
 * @psalm-method Collection&\Traversable<ChildNews> findByHeadline(string|array<string> $headline) Return ChildNews objects filtered by the headline column
 *
...
abstract class NewsQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \News\Base\NewsQuery object.
     *
     * @param string $dbName The database name
     * @param string $modelName The phpName of a model, e.g. 'Book'
     * @param string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\News\\News', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, headline FROM news WHERE id = :p0';
        try {
     ...
     * $query->filterByHeadline(['foo', 'bar']); // WHERE headline IN ('foo', 'bar')
     * </code>
     *
    ...
    public function filterByHeadline($headline = null, ?string $comparison = null)
    {
        ...
        $this->addUsingAlias(NewsTableMap::COL_HEADLINE, $headline, $comparison);
